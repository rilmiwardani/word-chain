...
try { WebcastPushConnection = require("tiktok-live-connector").WebcastPushConnection; } catch(e) { console.warn("[WARN] tiktok-live-connector not installed. TikTok features disabled."); }
...
// ═══════════════════════════════════════════════════════
//  NATIVE WEBSOCKET BRIDGE (port 62024 for game frontend)
// ═══════════════════════════════════════════════════════
const wss = new WebSocketServer({ port: WS_BRIDGE_PORT });
...
wss.on("connection", (ws) => {
  wsClients.add(ws);
  console.log(`[WS Bridge] Client connected (${wsClients.size} total)`);
  ...
  ws.on("close", () => {
    wsClients.delete(ws);
    console.log(`[WS Bridge] Client disconnected (${wsClients.size} total)`);
  });
});

console.log(`[WS Bridge] Native WebSocket server on port ${WS_BRIDGE_PORT}`);

// ═══════════════════════════════════════════════════════
//  MUSIC QUEUE MANAGEMENT
// ═══════════════════════════════════════════════════════
async function handleMusicSearch(query, requesterName, requesterImg) {
  try {
    ...
    if (r.videos.length > 0) {
      ...
      console.log(`[Music] Queued: "${song.title}" by ${requesterName}`);
      broadcastMusicState();
    }
  } catch (err) {
...
io.on("connection", (socket) => {
  ...
  socket.on("host-music-request", async (query) => {
    ...
    await handleMusicSearch(query, "Host", null);
  });
...
async function connectToTikTok(uniqueId) {
  ...
  try {
    ...
    if (tiktokSessionId) {
      ...
      process.env.WHITELIST_AUTHENTICATED_SESSION_ID_HOST = "tiktok.eulerstream.com"; // Bypass security warning
...
function disconnectFromTikTok() {
  ...
  io.emit("statusUpdate", getStatusPayload());
  wsBroadcast("tiktok_disconnected", { reason: "manual_disconnect" });
}
