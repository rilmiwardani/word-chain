...
import { Users, Clock, Settings, Trophy, Star, Plus, Minus, X, Gamepad2, FileJson, Skull, Bot, User, Keyboard, Send, Globe, Maximize, Minimize, Delete, Volume2, VolumeX, Medal, TrendingUp, AlertTriangle, MapPin, Music, Zap, Target } from 'lucide-react';

// ... (BAGIAN ATAS DARI DICTIONARY, CITIES, UTILS, SOUND MANAGER TETAP SAMA) ...
...
const DYNAMIC_CHALLENGES = [
    { id: 'NO_A', label: "🚫 No 'A'", check: w => !w.includes('a') },
    { id: 'NO_I', label: "🚫 No 'I'", check: w => !w.includes('i') },
    { id: 'NO_U', label: "🚫 No 'U'", check: w => !w.includes('u') },
    { id: 'NO_E', label: "🚫 No 'E'", check: w => !w.includes('e') },
    { id: 'NO_O', label: "🚫 No 'O'", check: w => !w.includes('o') },
    { id: 'HAS_S', label: "✅ Must have 'S'", check: w => w.includes('s') },
    { id: 'HAS_K', label: "✅ Must have 'K'", check: w => w.includes('k') },
    { id: 'HAS_R', label: "✅ Must have 'R'", check: w => w.includes('r') },
    { id: 'MIN_5', label: "📏 Min 5 Letters", check: w => w.length >= 5 },
    { id: 'MAX_5', label: "📏 Max 5 Letters", check: w => w.length <= 5 },
    { id: 'END_VOWEL', label: "🔤 End: Vowel", check: w => /[aeiou]$/i.test(w) },
    { id: 'END_CONS', label: "🔤 End: Consonant", check: w => /[^aeiou]$/i.test(w) },
    { id: 'ODD', label: "1️⃣3️⃣5️⃣ Odd Length", check: w => w.length % 2 !== 0 },
    { id: 'EVEN', label: "2️⃣4️⃣6️⃣ Even Length", check: w => w.length % 2 === 0 },
];
...
export default function WordChainGame() {
  ...
  const [gameDuration, setGameDuration] = useState(60); // Default 60s
  const [targetScore, setTargetScore] = useState(50); // Default Target Score
  const [winCondition, setWinCondition] = useState('TIME'); // 'TIME' or 'SCORE'
  ...
  function getSuffixOrRule(word) {
      ...
      // UPDATED: Suffix logic including POINT_RUSH_2
      ...
      if (gameMode === 'LAST_2_LETTERS' || gameMode === 'POINT_RUSH_2') return word.slice(-2); // Updated
  ...
  function validateConnection(prev, next) {
    ...
    // UPDATED: Dynamic Mode Validation
    if (gameMode === 'DYNAMIC') {
        // 1. Base connection: Last Letter
        const suffix = p.slice(-1);
        if (!n.startsWith(suffix)) return false;
        
        // 2. Dynamic Challenge Check
    ...
    // UPDATED: Validation for LAST_2_LETTERS and POINT_RUSH_2
  ...
  // UPDATED: Global Timer for POINT_RUSH (including 2) with Win Condition logic
  useEffect(() => {
    if (gameState === 'PLAYING' && (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && winCondition === 'TIME') {
        ...
        const interval = setInterval(() => {
            setGlobalTimer(prev => {
                ...
                return prev !== null ? prev - 1 : gameDuration;
            });
        }, 1000);
  ...
  function submitAnswer(word) {
    ...
    if (isValid) {
      ...
      // POINT RUSH LOGIC (Updated to include POINT_RUSH_2)
      if (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') {
          ...
          setPlayers(prev => prev.map((p, index) => {
              if (index === currentTurnIndex) {
                  ...
                  return { ...p, score: newScore }; // Return NEW object with updated score
              }
              return p;
          }));
          ...
          // Check win condition using current score + new points
          const currentScore = players[currentTurnIndex].score || 0;
          if (winCondition === 'SCORE' && (currentScore + word.length) >= targetScore) {
              setGameState('ENDED');
              playSound('win');
              addLog('System', `WINNER: ${players[currentTurnIndex].nickname} (Target Reached)! 🎉`);
              return; // Stop game
          }
      ...
      // UPDATED: Dynamic Rule Switching
      if (gameMode === 'DYNAMIC' && gameState !== 'ENDED') {
          // Increment valid turns
          setTurnCount(prev => {
             const nextTurn = prev + 1;
             const activePlayersCount = playersRef.current.filter(p => !p.isEliminated).length;
             // Switch rule if everyone played once (approx)
             if (nextTurn % Math.max(1, activePlayersCount) === 0) {
                 const newChallenge = DYNAMIC_CHALLENGES[Math.floor(Math.random() * DYNAMIC_CHALLENGES.length)];
                 setActiveChallenge(newChallenge);
                 addLog('System', `🚨 RULE CHANGE: ${newChallenge.label} 🚨`);
                 playSound('tick'); // Sound cue for rule change
             }
             return nextTurn;
          });
      }
  ...
  function startGame() {
    ...
    // UPDATED: Init Dynamic Challenge
    if (gameMode === 'DYNAMIC') {
        const firstChallenge = DYNAMIC_CHALLENGES[Math.floor(Math.random() * DYNAMIC_CHALLENGES.length)];
        setActiveChallenge(firstChallenge);
        setTurnCount(0);
        addLog('System', `Mode: DYNAMIC CHAOS!`);
        addLog('System', `Rule: ${firstChallenge.label}`);
    }

    setCurrentWord(randomStart);
    
    // UPDATED: Randomize Starting Player for Fairness
  ...
  function resetGame() {
      ...
      setPlayers(prev => prev.map(p => ({ ...p, isEliminated: false, score: 0 }))); 
  ...
  function cycleGameMode() {
      // UPDATED: Added DYNAMIC and POINT_RUSH_2 to the cycle
  ...
  function getModeLabel() {
      switch(gameMode) {
          ...
          case 'POINT_RUSH_2': return 'POINT RUSH (2)'; // UPDATED
          ...
          case 'DYNAMIC': return 'DYNAMIC'; // UPDATED
  ...
  // UPDATED: Separated Timer Logic to Fix Double Execution Bug
  useEffect(() => {
    if (gameState === 'PLAYING') {
      clearInterval(timerRef.current);
      setTimer(turnDuration); // Reset timer at start of turn
      
      timerRef.current = setInterval(() => {
        setTimer(prev => Math.max(0, prev - 1)); // Pure state update
      }, 1000);
    } else {
      clearInterval(timerRef.current);
    }
  ...
  // NEW: Effect to handle Timer side effects (Sound & Timeout)
  useEffect(() => {
  ...
  // UPDATED: Fixed Bug where eliminated high-scorer wins
  const getWinner = () => {
      if (gameState !== 'ENDED') return null;
      
      // 1. Get only active players (Survivors)
      const activePlayers = players.filter(p => !p.isEliminated);

      // If everyone is eliminated (Draw/Rare case), maybe return null or handle differently
      if (activePlayers.length === 0) return null;

      // 2. Logic for Point Rush
      if (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') {
          // Sort ONLY the active players by score
          // If only 1 player is left (Survivor), they win regardless of score (because they are the only one in the array)
          return [...activePlayers].sort((a, b) => (b.score || 0) - (a.score || 0))[0];
      }

      // 3. Logic for Standard Modes (Survivor wins)
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-900 text-white font-sans overflow-hidden flex flex-col items-center justify-center p-2 sm:p-4 relative">
      ...
      <div className="absolute top-2 right-2 sm:top-4 sm:right-4 z-50 flex flex-col items-end">
        ...
        `}>
            <div className="bg-slate-800/90 backdrop-blur-md p-3 rounded-lg border border-slate-600 shadow-2xl flex flex-col gap-3 min-w-[200px]">
                ...
                {/* Game End Condition (Time vs Score) - NEW */}
                <div className="w-full bg-slate-700 p-2 rounded border border-slate-600 flex flex-col gap-2">
                     <div className="flex items-center justify-between text-xs text-slate-300 font-bold">
                        ...
                        <button onClick={() => setWinCondition(prev => prev === 'TIME' ? 'SCORE' : 'TIME')} className="text-blue-400 hover:text-white transition-colors uppercase">{winCondition}</button>
                     </div>
                ...
                return (
                    ...
                    >
                        ...
                        <div className={`mt-[-12px] flex flex-col items-center z-40 relative`}> {/* Negative margin to overlap avatar */}
                            ...
                            `}>
                                {/* Trophy Section - MOVED OUTSIDE TERNARY */}
                                {player.stats?.wins > 0 && (
      ...
      {/* GIFT POPUP - UPDATED DESIGN (MOVED TO TOP RIGHT) */}
