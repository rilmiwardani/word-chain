...
import { Users, Clock, Settings, Trophy, Star, Plus, Minus, X, Gamepad2, FileJson, Skull, Bot, User, Keyboard, Send, Globe, Maximize, Minimize, Delete, Volume2, VolumeX, Medal, TrendingUp, AlertTriangle, MapPin, Music, Zap, Target, RefreshCw } from 'lucide-react';

// ... (BAGIAN DICTIONARY & CITIES TETAP SAMA SEPERTI SEBELUMNYA) ...
// ... (Kode bagian kamus dan data tidak berubah, disembunyikan untuk ringkas) ...
...
const DYNAMIC_CHALLENGES = [
    // TIER 1: EASY (Basic Constraints)
    { id: 'MIN_4', label: "📏 Min 4 Letters", check: w => w.length >= 4, tier: 1 },
    { id: 'MAX_6', label: "📏 Max 6 Letters", check: w => w.length <= 6, tier: 1 },
    { id: 'ODD', label: "1️⃣3️⃣5️⃣ Odd Length", check: w => w.length % 2 !== 0, tier: 1 },
    { id: 'EVEN', label: "2️⃣4️⃣6️⃣ Even Length", check: w => w.length % 2 === 0, tier: 1 },
    { id: 'HAS_A', label: "✅ Must have 'A'", check: w => w.includes('a'), tier: 1 },

    // TIER 2: MEDIUM (Letter Requirements)
    { id: 'HAS_S', label: "✅ Must have 'S'", check: w => w.includes('s'), tier: 2 },
    { id: 'HAS_K', label: "✅ Must have 'K'", check: w => w.includes('k'), tier: 2 },
    { id: 'HAS_R', label: "✅ Must have 'R'", check: w => w.includes('r'), tier: 2 },
    { id: 'END_VOWEL', label: "🔤 End: Vowel", check: w => /[aeiou]$/i.test(w), tier: 2 },
    { id: 'END_CONS', label: "🔤 End: Consonant", check: w => /[^aeiou]$/i.test(w), tier: 2 },
    { id: 'NO_I', label: "🚫 No 'I'", check: w => !w.includes('i'), tier: 2 },
    { id: 'NO_U', label: "🚫 No 'U'", check: w => !w.includes('u'), tier: 2 },

    // TIER 3: HARD (Restrictions)
    { id: 'NO_A', label: "🚫 No 'A'", check: w => !w.includes('a'), tier: 3 },
    { id: 'NO_E', label: "🚫 No 'E'", check: w => !w.includes('e'), tier: 3 },
    { id: 'NO_O', label: "🚫 No 'O'", check: w => !w.includes('o'), tier: 3 },
    { id: 'NO_N', label: "🚫 No 'N'", check: w => !w.includes('n'), tier: 3 }, 
    { id: 'MUST_2_VOWELS', label: "🔤 Min 2 Vowels", check: w => (w.match(/[aeiou]/gi) || []).length >= 2, tier: 3 },
    
    // TIER 4: EXPERT (Complex Logic)
    { id: 'NO_DOUBLE', label: "🚫 No Double Letters", check: w => !/(.)\1/.test(w), tier: 4 }, 
    { id: 'UNIQUE', label: "🌟 Unique Letters Only", check: w => new Set(w).size === w.length, tier: 4 },
    { id: 'SECOND_VOWEL', label: "🔤 2nd Letter: Vowel", check: w => w.length > 1 && /[aeiou]/.test(w[1]), tier: 4 },
    { id: 'SECOND_CONS', label: "🔤 2nd Letter: Consonant", check: w => w.length > 1 && /[^aeiou]/.test(w[1]), tier: 4 },
];
...
const StatsManager = {
    ...
    update: (uniqueId, isWin, incrementGame = true) => {
        try {
            const data = localStorage.getItem('word_chain_stats');
...
export default function WordChainGame() {
  ...
  const getNextChallenge = (queue, nextStartLetter) => {
      let tempQueue = [...queue];
      ...
      for (let i = 0; i < tempQueue.length; i++) {
          ...
          let isSafe = true;
          if (challenge.id.startsWith('NO_')) {
              const forbiddenChar = challenge.id.split('_')[1].toLowerCase();
              if (nextStartLetter === forbiddenChar) isSafe = false;
          }
          if (isSafe) {
  ...
  function getSuffixOrRule(word) {
      ...
      if (gameMode === 'LAST_2_LETTERS' || gameMode === 'POINT_RUSH_2') return word.slice(-2);
  ...
  function getRuleDisplay(word) {
    ...
    if (gameMode === 'CITIES') {
        ...
        return { label: "Point Rush", target: target, desc: `1 Letter = 1 Point`, action };
    }
    else if (gameMode === 'POINT_RUSH_2') {
        return { label: "Point Rush (2 Let)", target: target, desc: `Start with '${target}' (1 Let=1 Pt)`, action };
    }
    else if (gameMode === 'DYNAMIC') {
        return { 
            label: "Dynamic Chaos", 
            target: target, 
            desc: activeChallenge ? `${activeChallenge.label} + Starts '${target}'` : `Starts '${target}'`, 
            action 
        };
    }
  ...
  function validateConnection(prev, next) {
    ...
    if (gameMode === 'DYNAMIC') {
        ...
        if (activeChallenge && activeChallenge.check) {
            if (!activeChallenge.check(n)) {
                addLog('Game', `⚠️ Failed: ${activeChallenge.label}`);
                return false;
            }
        }
        return true;
    ...
    if (gameMode === 'CITIES' || gameMode === 'LAST_LETTER' || gameMode === 'POINT_RUSH') {
    ...
    else if (gameMode === 'LAST_2_LETTERS' || gameMode === 'POINT_RUSH_2') {
  ...
  // BUG FIX: Centralized Win Logic to update Stats
  const handleWin = (winner) => {
      // 1. Update Stats in LocalStorage (Win + 1, GameCount not incremented again here)
      // Note: We assume GameCount +1 happened on Join/Start
      const updatedStats = StatsManager.update(winner.uniqueId, true, false);
      
      // 2. Update Player State so UI reflects badges immediately
      setPlayers(prev => prev.map(p => 
          p.uniqueId === winner.uniqueId 
  ...
  // BUG FIX: Separate effect for Time Win to avoid setState inside render/other setters
  useEffect(() => {
    if (gameState === 'PLAYING' && (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && winCondition === 'TIME') {
  ...
  function handleTimeout() {
    ...
    // CHECK WIN CONDITION FOR ROUNDS 
    if ((gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && winCondition === 'ROUNDS') {
        ...
        if (activePlayers.length > 0 && activePlayers.every(p => (p.turnCount || 0) >= targetRounds)) {
            setGameState('ENDED');
            playSound('win');
            // Note: In Round/Time modes with multiple winners based on score, we might want to handle stats differently.
            // For now, let the user see the score board. Stats are updated when we explicitly call handleWin later or use a different trigger? 
            // In Point Rush, 'Win' usually goes to highest score.
            // Let's defer stats update to the 'ENDED' screen or calculation?
            // Ideally we calculate winner NOW.
    ...
    if (activePlayers.length <= 1) {
        if (activePlayers.length === 1) handleWin(activePlayers[0]);
             ...
             setGameState('ENDED'); // Draw/Everyone died
             playSound('win');
        }
    } else {
  ...
  function submitAnswer(word) {
    ...
    if (isValid) {
      ...
      // POINT RUSH LOGIC
      if (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') {
          let gameEnded = false;
          
          const newPlayersList = players.map((p, index) => {
              if (index === currentTurnIndex) {
                  const newScore = (p.score || 0) + word.length;
          ...
          // 1. SCORE TARGET
          if (winCondition === 'SCORE' && newPlayersList[currentTurnIndex].score >= targetScore) {
              gameEnded = true;
          }
          // 2. ROUNDS TARGET 
          ...
          if (gameEnded) {
             // Find winner (usually current player in Score mode, or high score in Rounds mode)
             const winner = [...newPlayersList].filter(p => !p.isEliminated).sort((a, b) => (b.score || 0) - (a.score || 0))[0];
             handleWin(winner);
             return;
          }
          
          addLog('Game', `✅ +${word.length} Pts!`);
      ...
      if (gameMode === 'DYNAMIC' && gameState !== 'ENDED') {
          setTurnCount(prev => {
             const nextTurn = prev + 1;
             const activePlayersCount = playersRef.current.filter(p => !p.isEliminated).length;
             if (nextTurn % Math.max(1, activePlayersCount) === 0) {
                 const nextStartLetter = word.slice(-1).toLowerCase();
                 const { selected, newQueue } = getNextChallenge(challengeQueueRef.current, nextStartLetter);
                 
                 setActiveChallenge(selected);
                 setChallengeQueue(newQueue);
                 
                 addLog('System', `🚨 RULE CHANGE: ${selected.label} 🚨`);
                 playSound('tick'); 
             }
             return nextTurn;
          });
  ...
  function joinGame(uniqueId, nickname, profilePictureUrl, isBot = false) {
    ...
    setPlayers(prev => {
      ...
      if (!isBot) {
         ...
         // BUG FIX: Pass true for incrementGame, false for isWin
         stats = StatsManager.update(uniqueId, false, true); 
      }
  ...
  function startGame() {
    ...
    // FIX 1: Pastikan semua pemain di-revive (isEliminated: false)
    setPlayers(prev => prev.map(p => ({...p, score: 0, turnCount: 0, isEliminated: false})));
    
    if ((gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && winCondition === 'TIME') {
    ...
    if (gameMode === 'DYNAMIC') {
        ...
        addLog('System', `Rule: ${selected.label}`);
    }
    ...
    // FIX 2: Reset Timer SEBELUM masuk state PLAYING agar tidak langsung kena timeout
    ...
    addLog('System', `Start: ${starterName} goes first!`);
  ...
  function resetGame() {
      ...
      setCurrentWord('');
      setGlobalTimer(null);
      // FIX 3: Reset timer juga saat reset game
      setTimer(turnDuration);
  ...
  function cycleGameMode() {
      const modes = ['LAST_LETTER', 'DYNAMIC', 'SECOND_LETTER', 'RHYME', 'POINT_RUSH', 'POINT_RUSH_2', 'LAST_2_LETTERS', 'LAST_3_LETTERS', 'SYLLABLE', 'LONGER_WORD', 'LONGER_2_LETTERS', 'CITIES'];
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-slate-900 text-white font-sans overflow-hidden flex flex-col items-center justify-center p-2 sm:p-4 relative">
      ...
      {/* GLOBAL TIMER/GOAL FOR POINT RUSH */}
      {gameState === 'PLAYING' && (gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-top fade-in">
              {winCondition === 'TIME' ? (
                  <div className={`px-6 py-2 rounded-full font-mono text-2xl font-bold border-2 shadow-[0_0_20px_currentColor] 
                      ${globalTimer !== null && globalTimer <= 10 ? 'text-red-500 border-red-500 animate-pulse bg-red-950/50' : 'text-blue-400 border-blue-500 bg-slate-900/80'}`}>
                      {globalTimer !== null ? `${Math.floor(globalTimer / 60)}:${(globalTimer % 60).toString().padStart(2, '0')}` : '0:00'}
                  </div>
              ) : winCondition === 'SCORE' ? (
                  <div className="px-6 py-2 rounded-full font-bold border-2 text-yellow-400 border-yellow-500 bg-slate-900/80 shadow-[0_0_20px_currentColor] flex items-center gap-2">
                      <Target className="w-5 h-5" />
                      Target: {targetScore}
                  </div>
              ) : (
                  <div className="px-6 py-2 rounded-full font-bold border-2 text-purple-400 border-purple-500 bg-slate-900/80 shadow-[0_0_20px_currentColor] flex items-center gap-2">
                      <RefreshCw className="w-5 h-5" />
                      Round: {Math.min(...players.filter(p => !p.isEliminated).map(p => p.turnCount || 0)) + 1} / {targetRounds}
                  </div>
              )}
          </div>
      )}
            ...
            <div className={`absolute inset-0 rounded-full border-[8px] bg-slate-800 transition-all duration-200 flex items-center justify-center overflow-hidden ${getTableStatusClass()}`}>
                ...
                <div className="relative z-10 text-center flex flex-col items-center">
                    ...
                    <div>
                        ...
                        {(gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && <div className="text-[10px] font-bold bg-green-900/50 px-2 py-0.5 rounded text-green-300 mb-1 flex items-center justify-center gap-1"><Zap className="w-3 h-3"/> POINT RUSH {gameMode === 'POINT_RUSH_2' ? '(2)' : ''}</div>}
                        {gameMode === 'DYNAMIC' && <div className="text-[10px] font-bold bg-orange-900/50 px-2 py-0.5 rounded text-orange-300 mb-1 flex items-center justify-center gap-1"><AlertTriangle className="w-3 h-3"/> DYNAMIC CHAOS</div>}
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in duration-700">
            ...
            <div className="text-center transform transition-all animate-in zoom-in-50 duration-500 flex flex-col items-center">
                ...
                <div className="bg-slate-800/80 px-8 py-3 rounded-xl border border-yellow-500/50 backdrop-blur-md">
                    ...
                    {(gameMode === 'POINT_RUSH' || gameMode === 'POINT_RUSH_2') && (
                        <p className="text-lg font-mono text-green-300 mt-1">Score: {getWinner().score}</p>
                    )}
                </div>
              ...
              <div className="flex flex-col items-center relative">
                  ...
                  <div className="relative mb-2">
                      <div className="w-16 h-16 rounded-full p-1 bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 shadow-2xl">
                        <img 
                            src={lastEvent.profilePictureUrl || getAvatarUrl(lastEvent.nickname)} 
                            alt="Gifter" 
                            className="w-full h-full rounded-full object-cover border-2 border-slate-900 bg-slate-800" 
                        />
                      </div>
                      <div className="absolute -bottom-1 -right-1 bg-yellow-400 text-black font-black text-[10px] px-2 py-0.5 rounded-full shadow-lg border border-white transform rotate-6">
                        GIFT!
                      </div>
                  </div>
                  ...
                  <div className="bg-slate-900/90 border border-pink-500/50 backdrop-blur-xl px-4 py-2 rounded-xl shadow-2xl text-center transform hover:scale-105 transition-transform">
                      <p className="text-pink-300 text-[8px] font-bold tracking-widest uppercase mb-0.5">Supporter</p>
                      <div className="flex flex-col">
                          <span className="text-sm font-black text-white drop-shadow-md truncate max-w-[150px]">{lastEvent.nickname}</span>
                          <span className="text-xs text-slate-300">sent <span className="text-yellow-400 font-bold">{lastEvent.giftName}</span> 🎁</span>
                      </div>
                  </div>
              </div>
          </div>
